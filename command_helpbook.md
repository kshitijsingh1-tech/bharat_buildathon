...
> [!IMPORTANT]
> Running `git config user.email` or `git config user.name` ONLY changes the name/email written on commit labels. It does **NOT** change the GitHub account used for HTTPS authentication.
> When Git displays `info: please complete authentication in your browser...`, it opens your default browser. If your browser is already logged into `manthan-hex` on [github.com](https://github.com), GitHub automatically authorizes Git under that logged-in user!

**How to switch to `kshitijsingh1-tech` (or another account):**

* **Method 1: Log out in Browser & Reset Credentials**
  1. Open your web browser, navigate to [https://github.com](https://github.com), and **log out** of `manthan-hex`. Log into `kshitijsingh1-tech`.
  2. Clear cached credentials in PowerShell / CMD:
     ```cmd
     cmdkey /delete:git:https://github.com
     ```
  3. Re-run your push command:
     ```bash
     git push -u origin main
     ```
  4. The browser window will now authenticate as `kshitijsingh1-tech`.

* **Method 2: Authenticate using a GitHub Personal Access Token (PAT)**
  1. Go to **GitHub Settings $\rightarrow$ Developer Settings $\rightarrow$ Personal Access Tokens $\rightarrow$ Tokens (classic)** while logged into `kshitijsingh1-tech`.
  2. Generate a token with the `repo` scope enabled and copy the token string.
  3. Set your remote URL with the token included:
     ```bash
     git remote set-url origin https://YOUR_TOKEN@github.com/kshitijsingh1-tech/bharat_buildathon.git
     ```
  4. Push directly without browser prompts:
     ```bash
     git push -u origin main
     ```
...
1. Go to your repository on GitHub (`https://github.com/kshitijsingh1-tech/bharat_buildathon`).

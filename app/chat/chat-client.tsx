...
export function ChatClient({ initialQuery, fullScreen = false }: { initialQuery?: string; fullScreen?: boolean }) {
  ...
  const processInput = (text: string, currentProfile: UserProfile) => {
    // Parse input
    ...
    if (!parsed.state) {
      ...
      // Check if the text itself is a state name in the schemes
      for (const [key, val] of Object.entries(STATE_MAP)) {
        if (direct.toLowerCase() === key || direct === val) { parsed.state = val; break }
      }
    }

    // Handle age range chips like "18–25 years" / "18–25 साल"
    if (!parsed.age) {
    ...
    // Handle income range chips like "Below ₹1,00,000"
    if (!parsed.income) {
      if (/below|कम|से कम/i.test(text)) {
        ...
        // Try range like "₹1,00,000 – ₹2,50,000"
    ...
    // Handle profession chips (direct match)
    ...
    // Handle need chips (direct match)
    if (!parsed.need) {
      for (const [need, keywords] of Object.entries(NEED_KEYWORDS)) {
      ...
      // Direct match for category names
    ...
    if (nextField) {
      ...
      const confirmText = Object.keys(parsed).length > 0
        ? (hi ? '✓ समझ गया। ' : '✓ Got it. ')
        : ''
      setMessages((m) => [
        ...m,
      ...
      // All info collected — find matching schemes
